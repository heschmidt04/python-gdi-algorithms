...
tic_tac_toe_row = [[1, 1, 1], [0, 2, 0], [2, 0, 2]]

tic_tac_toe_row_too = [[0, 2, 1], [1, 1, 1], [2, 0, 2]]
...
tic_tac_toe_diag = [[2, 0, 1], [0, 1, 2], [1, 0, 2]]
...
def tic_tac_toe_win_check(board, column=None):
    ...
    for row in board:
        ...
        # What is happening is that it does not
        # increment through the rest of the columns - it does col 0 and then ends
    # Adding a column counter to see if it increments
    for column in range(len(board)):
        win = check_column_win(column, board)
        print(
            f"This is what win is set to from check_column_win: {win} "
            f"for the column and board position of {column}, in {board}"
        )
